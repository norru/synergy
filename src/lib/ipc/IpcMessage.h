...
public:
	IpcLogLineMessage(const nstring& logLine);
	virtual ~IpcLogLineMessage();

	//! Gets the log line.
	nstring				logLine() const { return m_logLine; }
...
public:
	IpcCommandMessage(const nstring& command, bool elevate);
	virtual ~IpcCommandMessage();

	//! Gets the command.
	nstring				command() const { return m_command; }
