...
void
XWindowsClipboard::add(EFormat format, const std::string& data)
{
	assert(m_open);
	assert(m_owner);
		...
		if (CLOG->getFilter() >= kDEBUG2) {
			...
			for (int i = 0; i < n; ++i) {
				...
				if (!XWindowsUtil::getWindowProperty(m_display,
					...
					for (nstring::size_type j = 0; j < data.size(); ++j) {
...
Atom
XWindowsClipboard::getTargetsData(std::string& data, int* format) const
{
	assert(format != NULL);
...
Atom
XWindowsClipboard::getTimestampData(std::string& data, int* format) const
{
	assert(format != NULL);
