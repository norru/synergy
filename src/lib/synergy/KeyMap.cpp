...
KeyMap::addKeyEntry(const KeyItem& item)
{
	...
	LOG((CLOG_DEBUG5"add key: %04x %d %03x %04x (%04x %04x %04x)%" _NF),
		newItem.m_id, newItem.m_group, newItem.m_button, newItem.m_client,
		newItem.m_required, newItem.m_sensitive, newItem.m_generates, newItem.m_dead ?" dead") :"")));
}
				...
				Keystrokes& keystrokes) const
{
	...
	if (keyItem.m_dead) {
		...
		if (!keysForModifierState(keyItem.m_button, group,
			...
			LOG((CLOG_DEBUG1"unable to match desired modifier state (%04x,%04x) for key %d"),
				...
				Keystrokes& keystrokes) const
{
	...
	for (SInt32 bit = kKeyModifierNumBits; bit-- > 0; ) {
		...
		if ((currentState & sensitive) != (keyItem->m_required & sensitive)) {
			LOG((CLOG_DEBUG1"unable to match modifier state for modifier %04x (%04x vs %04x in %04x)"),
...
KeyMap::formatKey(KeyID key, KeyModifierMask mask)
{
	...
	if (key != kKeyNone) {
		...
		// XXX -- we're assuming ASCII here
			...
			x += synergy::string::sprintf(N"\\u%04x"), key);
		}
	}
	else if (!x.empty()) {
		// remove trailing '+'
...
KeyMap::parseModifiers(nstring& x, KeyModifierMask& mask)
{
	...
	mask = 0;
	nstring::size_type tb = x.find_first_not_of(N" \t"), 0);
	while (tb != nstring::npos) {
		// get next component
		nstring::size_type te = x.find_first_of(N" \t+)"), tb);
		if (te == nstring::npos) {
			te = x.size();
		}
		...
		if (s_nameToModifierMap->count(c) > 0) {
			...
			// unknown string
			x.erase(0, tb);
			nstring::size_type tb = x.find_first_not_of(N" \t"));
			nstring::size_type te = x.find_last_not_of(N" \t"));
			if (tb == nstring::npos) {
				x ="");
			}
			else {
		...
		tb = x.find_first_not_of(N" \t"), te);
		if (tb != nstring::npos) {
			if (x[tb] !='+')) {
				// expected '+'
				return false;
			}
			tb = x.find_first_not_of(N" \t"), tb + 1);
		}
	}
