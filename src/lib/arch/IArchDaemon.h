...
public:
	typedef int			(*DaemonFunc)(int argc, const nchar** argv);
	...
	*/
	virtual void		installDaemon(const nchar* name,
							...
							const nchar* dependencies) = 0;

	//! Uninstall daemon
	/*!
	Uninstall a daemon.  Throws an \c XArchDaemon on failure.
	*/
	virtual void		uninstallDaemon(const nchar* name) = 0;
	...
	*/
	virtual int			daemonize(const nchar* name, DaemonFunc func) = 0;
	...
	*/
	virtual bool		canInstallDaemon(const nchar* name) = 0;

	//! Check if the daemon is installed
	/*!
	Returns true iff the daemon is installed.
	*/
	virtual bool		isDaemonInstalled(const nchar* name) = 0;
	...
	*/
	virtual nstring		commandLine() const = 0;
