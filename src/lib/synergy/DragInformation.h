...
public:
	...
	nstring&			getFilename() { return m_filename; }
	void				setFilename(nstring& name) { m_filename = name; }
	...
	static nstring		getDragFileExtension(nstring filename);
	...
	static bool			isFileValid(nstring filename);

private:
	static size_t		stringToNum(std::string& str);
	static std::string	getFileSize(nstring& filename);
